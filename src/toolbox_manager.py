...
EXCLUDED_DIRS = {"venv", ".venv", ".git", "__pycache__", "build", "dist", ".ipynb_checkpoints","legacy","_legacy","common_libs","__init__.py","toolbox_manager.py","aux_"}
APP_DIR = os.path.dirname(os.path.abspath(__file__))
INI_PATH = os.path.join(APP_DIR, "tools_catalog.ini")
LOG_FOLDER_PATH = "k:/_provenance_phis/Logs"
...
class Catalog:
    def __init__(self, ini_path: str, scan_root: str = ""):
        ...
        self.load()
        self.global_settings["log_dir"] = LOG_FOLDER_PATH
    ...
    def _entry_from_section(self, section: str) -> ToolEntry:
        ...
        return ToolEntry(
            ...
            highlight=sec.getboolean("highlight", False),
            color=sec.get("color", ""),
        )
    ...
    def save(self):
        ...
        cfg["app"] = {
            ...
            "log_dir": self.global_settings.get("log_dir", ""),
        }
        # tool sections
        for rel_path, ent in sorted(self.tools.items()):
            sec = {
                ...
                "highlight": str(ent.highlight).lower(),
                "color": ent.color,
            }
            cfg[rel_path] = sec
            # presets
    ...
    def get_log_dir(self) -> str:
        """
        Returns toolbox-managed global log directory.
        Ensures directory exists.
        """
        log_dir = (self.global_settings.get("log_dir") or "").strip()

        # Default fallback
        if not log_dir:
            if self.scan_root:
                log_dir = os.path.join(self.scan_root, "logs")
            else:
                log_dir = os.path.join(os.getcwd(), "logs")

            # store fallback so it persists
            self.global_settings["log_dir"] = log_dir
            self.save()
...
class App(tk.Tk):
    ...
    def _refresh_single_row(self, rel):
    ...
    def _apply_highlight_change(self):
        if not self.selected_tool_id:
            return

        ent = self.catalog.tools[self.selected_tool_id]
        ent.highlight = self.highlight_var.get()

        self._refresh_single_row(self.selected_tool_id)
        self.catalog.save()
    ...
    def _choose_color(self):
        ...
        # Save immediately (optional but recommended)
        self.catalog.save()

    def _load_icons(self):
        from tkinter import PhotoImage
    
        self.icon_py = PhotoImage(width=16, height=16)
        self.icon_py.put("#FFD43B", to=(0,0,15,15))  # yellow square placeholder
    
        self.icon_default = PhotoImage(width=16, height=16)
        self.icon_default.put("#AAAAAA", to=(0,0,15,15))  # gray square
    
        # IMPORTANT: keep references
        self._icon_refs = [self.icon_py, self.icon_default]

    
    def _on_double_click_run(self, event):
        item = self.tree.identify_row(event.y)
        if not item:
            return

        # Select the row properly
        self.tree.selection_set(item)
        self.tree.focus(item)
        self.selected_tool_id = item
    ...
    def _build_ui(self):
        ...
        main_split.add(left, weight=4)
        main_split.add(right, weight=1)

        self.update_idletasks()
        main_split.sashpos(0, int(self.winfo_width() * 0.70))
        # Table
        ...
        self.tree.tag_configure("bold", font=("Segoe UI", 10, "bold"))

        self.tree.heading("#0", text="Tool")
        self.tree.column("#0", width=900, anchor=tk.W)
        ...
        self.tree.bind("<Double-1>", self._on_double_click_run)
        ...
        # Highlight + Color
        style_fr = ttk.Frame(meta)
        style_fr.pack(fill=tk.X, padx=4, pady=2)
        ...
        ttk.Checkbutton(
            ...
            command=self._apply_highlight_change
        ).pack(side=tk.LEFT)


        ttk.Label(style_fr, text="Color:").pack(side=tk.LEFT, padx=6)
        self.color_var = tk.StringVar()
        ttk.Entry(style_fr, textvariable=self.color_var, width=10).pack(side=tk.LEFT)

        ttk.Button(style_fr, text="Pick", command=self._choose_color).pack(side=tk.LEFT)
    ...
    def _insert_row_prv(self, rel: str):
        ent = self.catalog.tools[rel]
        vals = [
    ...
    def _insert_row(self, rel: str):
        ...
        icon = self.icon_py if rel.endswith(".py") else self.icon_default
        ...
        if ent.color:
            tag_name = f"color_{ent.color}"
            tags.append(tag_name)
            if not self.tree.tag_has(tag_name):
                self.tree.tag_configure(tag_name, foreground=ent.color)

        self.tree.insert(
            '',
            tk.END,
            iid=rel,
            text=ent.display_name(),
            image=icon,
            tags=tags
        )
    ...
    def on_select_tool(self, event=None):
        ...
        self.highlight_var.set(ent.highlight)
        self.color_var.set(ent.color)
    ...
    def on_run(self):
        ...
        # 🔑 Inject LOG_ENV_PATH for THIS tool only
        LOG_ENV_PATH = os.path.join(APP_DIR, "log_path.env")
        if os.path.isfile(LOG_ENV_PATH):
            env["LOG_ENV_PATH"] = LOG_ENV_PATH

        # Tool-specific env overrides from GUI (can override LOG_ENV_PATH)
        ...
        # Example: mypkg/tool.py → mypkg.tool
        module_path = ent.rel_path.replace("/", ".").replace("\\", ".")
        if module_path.endswith(".py"):
            module_path = module_path[:-3]

        # Ensure scan_root (src folder) is in PYTHONPATH
        env["PYTHONPATH"] = self.catalog.scan_root

        cmd = [python_exe, "-u", "-m", module_path] + arg_list


        # ----------------------------
        # Logging (Toolbox-managed)
        # ----------------------------
        toolname = ent.tool_name()
        tstamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

        # 🔹 Ask catalog for global log dir (managed by INI module)
        base_log_dir = self.catalog.get_log_dir()   # <-- this must exist in your new INI module

        tool_log_dir = os.path.join(base_log_dir, toolname)
    ...
    def _shell_split_prv(self, s: str) -> List[str]:
        # simple splitter honoring quotes
        return re.findall(r'\"([^\"]*)\"|\'([^\']*)\'|([^\s]+)', s)

    def _shell_split(self, s: str) -> List[str]:
        parts = re.findall(r'"([^"]*)"|\'([^\']*)\'|([^\s]+)', s)
        return [next(filter(None, p)) for p in parts]
