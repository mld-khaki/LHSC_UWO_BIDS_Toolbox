...
class EdfHeader:
    ...
    computed_file_size: int | None   # header_bytes + num_records * bytes_per_record
...
def parse_edf_header(path: str) -> EdfHeader:
    ...
    # Compute expected file size from header metadata
    computed_file_size: int | None = None
    if num_records > 0 and bytes_per_record > 0:
        computed_file_size = header_bytes + num_records * bytes_per_record
...
def format_header_for_display(h: EdfHeader) -> str:
    ...
    lines.append(f"File size (disk)     : {h.file_size} bytes")
    if h.computed_file_size is not None:
        lines.append(f"File size (computed) : {h.computed_file_size} bytes  (header + records × bytes/record)")
    else:
        lines.append(f"File size (computed) : N/A  (num_records unknown)")
...
class EDFComparatorApp:
    SUMMARY_FIELDS = [
        ...
        ("Full path", "path"),
        ("File size — disk (bytes)", "file_size"),
        ("File size — computed (bytes)", "computed_file_size"),
    ...
    def _build_pane(self, parent: ttk.Frame, side: str):
        ...
        if side == "RIGHT":
            ...
            sweep_btn = ttk.Button(
                top,
                text="Sweep sessions ↺",
                command=self._sweep_right
            )
            sweep_btn.pack(side="left", padx=2)
    ...
    # ------------------------------------------------------------------
    # Shared helper: find EDF/BDF files in a directory that match the
    # LEFT file by size.  Returns a list of matching absolute paths.
    # ------------------------------------------------------------------

    def _resolve_right_dir(self) -> str | None:
        """Return a valid directory from the RIGHT path_var, or None."""
        right_path = self.right.path_var.get().strip()
        if os.path.isdir(right_path):
            return right_path
        # Maybe path_var holds a file path → use its directory
        parent = os.path.dirname(right_path)
        if os.path.isdir(parent):
            return parent
        return None

    def _find_matches_by_size(self, left_size: int, right_dir: str) -> list[str]:
        """Return all EDF/BDF files in right_dir whose on-disk size == left_size."""
        matches = []
        try:
            entries = os.listdir(right_dir)
        except OSError:
            return matches
        for fname in entries:
            if not fname.lower().endswith((".edf", ".bdf")):
                continue
            full = os.path.join(right_dir, fname)
    ...
    def _find_corresponding_in_right(self):
        ...
        right_dir = self._resolve_right_dir()
        if not right_dir:
            messagebox.showerror(
        ...
        matches = self._find_matches_by_size(self.left.header.file_size, right_dir)
    ...
    def _sweep_right(self):
        """
        Sweep through BIDS sessions on the LEFT side looking for a file
        whose on-disk size matches a file in the RIGHT directory.

        Algorithm:
          1. Start from the currently loaded LEFT file (or first file if none).
          2. Try to find a size-matching file in the RIGHT directory.
          3. If found → load it in RIGHT and stop.
          4. If not found → advance LEFT to next BIDS session file.
          5. Repeat until a match is found or all sessions are exhausted.
        """
        if not self.left.header:
            messagebox.showerror("No LEFT file", "Load an EDF/BDF file in the LEFT pane first.")
            return
        ...
        if not right_dir:
            messagebox.showerror(
                "Invalid RIGHT directory",
                "The RIGHT pane must contain a directory path (enter or load a directory)."
            )
            return

        # Keep track of visited paths to avoid infinite loops
        visited: set[str] = set()
        current_left_path = self.left.header.path

        while True:
            # Guard against revisiting
            try:
                canonical = str(Path(current_left_path).resolve())
            except Exception:
                canonical = current_left_path

            if canonical in visited:
                break
            visited.add(canonical)

            # Try size match for current LEFT file
            try:
                left_size = os.path.getsize(current_left_path)
            except OSError:
                left_size = -1

            if left_size > 0:
                matches = self._find_matches_by_size(left_size, right_dir)
                if matches:
                    # Load the first match (or let user pick if multiple)
                    if len(matches) > 1:
                        sel = self._select_from_list(
                            title="Multiple matching files",
                            prompt=(
                                f"Multiple files match LEFT file:\n  {os.path.basename(current_left_path)}\n"
                                "Select one to load in RIGHT:"
                            ),
                            options=matches
                        )
                        if sel:
                            # Also load the matched LEFT file so both panes are in sync
                            self._load_from_explicit_path(self.left, current_left_path)
                            self._load_from_explicit_path(self.right, sel)
                        return
                    else:
                        self._load_from_explicit_path(self.left, current_left_path)
                        self._load_from_explicit_path(self.right, matches[0])
                        return

            # No match → advance LEFT to next BIDS session file
            next_path = self._compute_next_path_bids(current_left_path)
            if not next_path:
                next_path = self._compute_next_path_same_folder(current_left_path)

            if not next_path:
                break

            current_left_path = next_path

        messagebox.showinfo(
            "Sweep complete — no match",
            f"No EDF/BDF file in:\n  {right_dir}\n"
            "matched any LEFT file across all reachable BIDS sessions."
        )
