...
@author: Greydon
@author: Milad Khaki _2026_03_09___22_46_07_Mon
"""
import os
import numpy as np
import pandas as pd
...
import pdb
from annot_redact import anonymize_edf_complete
from helpers import init_logger


from ext_lib.edflibpy.edfreader_mld2 import EDFreader as EDFLIBReader
...
class edf2bids(QtCore.QRunnable):
    ...
    def _annotations_data(self, source_name, data_fname, callback, deidentify):
        ...
        """
        # Use a LOCAL header variable to avoid race conditions when multiple
        # worker threads call this method concurrently on the same instance.
        file_in = EDFReader()
        file_in.open(source_name)
        header = file_in.readHeader()   # local, not self.header
        ...
        ch_names = header.get('chan_info', {}).get('ch_names', [])
        tal_indx_list = [i for i, x in enumerate(ch_names) if x.endswith('Annotations')]
        tal_indx = tal_indx_list[0] if tal_indx_list else None
        callback.emit('...')
        ...
        if deidentify:
            ...
            if overwrite_match:
                ...
                if identity_idx and tal_indx is not None:
        ...
        if events:
            fulldate = datetime.datetime.strptime(header['meas_info']['meas_date'], '%Y-%m-%d %H:%M:%S')
            millisecond = float(header['meas_info'].get('millisecond', 0))  # default 0 if no TAL channel
            for iannot in events:
                data_temp = {
                    'onset': iannot[0] / 10000000,
                    'duration': iannot[1],
                    'time_abs': (fulldate + datetime.timedelta(seconds=(iannot[0] / 10000000) +
                                                                millisecond)).strftime(
                        '%H:%M:%S.%f'),
