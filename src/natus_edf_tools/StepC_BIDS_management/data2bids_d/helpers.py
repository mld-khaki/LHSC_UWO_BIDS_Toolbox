...
@author: Greydon
@author: Milad Khaki, _2026_03_09___22_45_48_Mon
"""
import os
import numpy as np
import pandas as pd
...
class EDFReader():
    ...
    def readHeader(self):
        try:
            ...
            tal_indx_list = [i for i,x in enumerate(self.header['chan_info']['ch_names']) if x.endswith('Annotations')]
            if tal_indx_list:
                tal_indx = tal_indx_list[0]
                annot_block = self.read_annotation_block(0, tal_indx)
                if annot_block and annot_block[0] and annot_block[0][0]:
                    self.header['meas_info']['millisecond'] = meas_info['millisecond'] = annot_block[0][0][0]
            return self.header
        except Exception as e:
            self.header = {}
            import traceback
            print(f"\n[EDFReader ERROR] File: {self.fname}")
            traceback.print_exc()   # ← this will print the real error to terminal
            raise(e)
    ...
    def overwrite_annotations(self, events, identity_idx, tal_indx, strings, action):
        ...
        print(f"readacted events: {redacted_events}")
