...
from natus_edf_tools.StepA_Natus_landing_ingestion.Quasar_EDFCompatCheck.EDF_Compatibility_Check_tool import check_edf_compatibility
...
def is_file_ready(filepath: str,
                  min_age_sec: int = FILE_STABILITY_AGE_SEC,
                  stable_window_sec: int = 10) -> bool:
    """
    File is ready ONLY IF:
    - exists
    - mtime older than min_age_sec
    - size stable across stable_window_sec
    - readable
    - not locked
    """
    ...
    # Too new → still hot
    if time.time() - stat2.st_mtime < min_age_sec:
        return False

    # Windows lock test
    if is_file_locked_via_rename(filepath):
        return False
