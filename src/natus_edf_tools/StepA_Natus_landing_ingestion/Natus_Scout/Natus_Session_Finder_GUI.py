...
# ----------------------------
# EDF Support (pyedflib)
# ----------------------------
# Try to import pyedflib for EDF file support
try:
    from common_libs.edflib_fork_mld.edfreader_mld2 import EDFreader, EDFexception
    EDF_AVAILABLE = True
except Exception:
    try:
        # Fallback to standard pyedflib
        import pyedflib
        EDF_AVAILABLE = True
        EDFreader = None  # Will use pyedflib directly
    except Exception:
        EDF_AVAILABLE = False
        EDFreader = None
...
def edf_extract_metadata(edf_path: Path, log=None) -> dict:
    """
    Extract metadata from an EDF file using pyedflib (or custom edflib_fork_mld).
    Returns a dict compatible with Natus FolderRow structure.

    Fixes:
    - correctly decodes bytearray/bytes
    - strips EDF fixed-width padding
    - collapses whitespace (no extra spaces)
    - removes non-printable junk
    - avoids useless default "EDF Recording" when real info exists
    """

    def _to_clean_str(x) -> str:
        """
        Convert EDF header field (str/bytes/bytearray/None/etc) to a clean string:
        - decodes bytes-like
        ...
        # Convert bytes-like -> str
        if isinstance(x, (bytes, bytearray)):
            try:
                s = bytes(x).decode("utf-8", errors="ignore")
            except Exception:
                try:
                    s = bytes(x).decode("latin-1", errors="ignore")
                except Exception:
                    s = str(x)
        else:
            s = str(x)

        # Remove nulls and non-printables (but keep normal chars)
        s = s.replace("\x00", " ")
        ...
        # Some EDF writers fill fields with just spaces
        if s == "":
            return ""

        return s

    def _join_clean(parts) -> str:
        """
        Join multiple EDF fields into one clean string.
        Skips empty/whitespace-only parts.
        """
        cleaned = []
        for p in parts:
            cp = _to_clean_str(p)
    ...
    def _pick_first_nonempty(*candidates) -> str:
        """
        Return first candidate that becomes non-empty after cleaning.
        """
        for c in candidates:
            cc = _to_clean_str(c)
            if cc:
    ...
    if not EDF_AVAILABLE:
        if log:
            log(f"[EDF] pyedflib not available for {edf_path.name}")
        return result

    try:
        if EDFreader is not None:
            # Use custom edflib_fork_mld
            r = EDFreader(str(edf_path), read_annotations=False)
            try:
                # Start date/time
                dt = r.getStartDateTime()
                result["RecordingStartTime"] = dt.strftime("%Y-%m-%d %H:%M:%S")
                ...
                result["RecordingEndTime"] = end_dt.strftime("%Y-%m-%d %H:%M:%S")

                # Patient info
                result["PatientName"] = _to_clean_str(r.getPatientName())
                result["PatientCode"] = _to_clean_str(r.getPatientCode())
                result["PatientSex"] = _to_clean_str(r.getPatientGender())
                result["PatientDOB"] = _to_clean_str(r.getPatientBirthDate())
                ...
                result["Machine"] = _join_clean(equipment_parts)

                # EegNo: prefer explicit EEG number if you have one, else patient code
                result["EegNo"] = result["PatientCode"]

                # StudyName: try multiple fields intelligently
                # Important: do NOT default to EDF Recording unless ALL are empty
                study_candidates = [
                    r.getRecordingAdditional(),
                    r.getAdministrationCode(),
                    r.getRecording(),
                    r.getReserved(),
                    r.getEquipment(),
                ]
                study = _join_clean(study_candidates)

                # Remove common junk default labels if present alone
                if study.lower() in {"edf recording", "recording"}:
                    study = ""

                result["StudyName"] = study if study else ""

                # If still empty, last-resort fallback:
                # use filename stem rather than useless "EDF Recording"
                if not result["StudyName"]:
                    result["StudyName"] = edf_path.stem

            finally:
                r.close()

        else:
            # Use standard pyedflib
            import pyedflib

            with pyedflib.EdfReader(str(edf_path)) as f:
                start_dt = f.getStartdatetime()
                result["RecordingStartTime"] = start_dt.strftime("%Y-%m-%d %H:%M:%S")

                duration_sec = f.getFileDuration()
                end_dt = start_dt + timedelta(seconds=duration_sec)
                result["RecordingEndTime"] = end_dt.strftime("%Y-%m-%d %H:%M:%S")

                patient_code = _to_clean_str(f.getPatientCode())
                result["PatientCode"] = patient_code
                result["PatientName"] = _to_clean_str(f.getPatientName())
                result["EegNo"] = patient_code

                # Machine
                result["Machine"] = _to_clean_str(f.getEquipment())

                # Study name
                # pyedflib often provides recordingAdditional, but it may be empty/padded
                study = _pick_first_nonempty(
                    f.getRecordingAdditional(),
                    f.getPatientAdditional(),
                    f.getEquipment(),
                )

                # Avoid useless defaults
                if study.lower() in {"edf recording", "recording"}:
                    study = ""

                result["StudyName"] = study if study else edf_path.stem

    except Exception as e:
        if log:
            log(f"[EDF error] {edf_path.name}: {e}")
        result["StudyName"] = f"ERROR: {type(e).__name__}"
        raise
...
def scan_directory(self):
    """Scan the selected directory for Natus folders, EDF files, and BIDS structure"""
    ...
    self.natus_folders.clear()
    self.edf_files.clear()
    self.bids_files.clear()
    self.unexpected_files.clear()
    
    try:
        # Get all items in directory
        all_items = set(os.listdir(directory))
        remaining_items = all_items.copy()
        
        # Stage 1: Scan for Natus folders
        for item in all_items:
            item_path = os.path.join(directory, item)
            if os.path.isdir(item_path):
                if self.is_natus_folder(item_path):
                    session_name = item
                    self.natus_folders[session_name] = item_path
                    remaining_items.discard(item)
        
        # Stage 2: Scan for EDF files in root directory
        for item in list(remaining_items):
            item_path = os.path.join(directory, item)
            if os.path.isfile(item_path) and item.lower().endswith('.edf'):
                self.edf_files[item] = item_path
                remaining_items.discard(item)
        
        # Stage 3: Recursively scan remaining folders for BIDS structure
        for item in remaining_items:
            item_path = os.path.join(directory, item)
            if os.path.isdir(item_path):
                self.scan_for_bids(item_path)
        
        # Populate the tree view
        self.populate_tree()
        
        # Update status
        total = len(self.natus_folders) + len(self.edf_files) + len(self.bids_files) + len(self.unexpected_files)
        self.status_label.config(
            text=f"Found: {len(self.natus_folders)} Natus folders, "
                 f"{len(self.edf_files)} root EDFs, "
                 f"{len(self.bids_files)} BIDS EDFs, "
                 f"{len(self.unexpected_files)} unexpected EDFs"
        )
...
def scan_for_bids(self, folder_path):
    """Recursively scan a folder for BIDS-formatted EDF files"""
    import re
    try:
        for root, dirs, files in os.walk(folder_path):
            for file in files:
                if file.lower().endswith('.edf'):
                    ...
                    # Check if it's BIDS format: starts with sub-###_ses-###
                    if self.is_bids_format(file):
                        # Extract session info from filename
                        session_info = self.extract_bids_session(file)
                        self.bids_files[file] = {
                            'path': file_path,
                            'session': session_info
                        }
                    else:
                        # Non-BIDS EDF file in subfolder - unexpected
                        self.unexpected_files[file] = file_path
    except Exception as e:
        print(f"Error scanning {folder_path}: {str(e)}")

def is_bids_format(self, filename):
    """Check if filename follows BIDS format: sub-###_ses-###_..."""
    import re
    pattern = r'^sub-\d+_ses-\d+_'
    return bool(re.match(pattern, filename))

def extract_bids_session(self, filename):
    """Extract session information from BIDS filename"""
    import re
    # Extract sub-### and ses-###
    ...
    if sub_match and ses_match:
        return f"sub-{sub_match.group(1)}_ses-{ses_match.group(1)}"
    return "Unknown"        
...
def populate_tree(self):
    """Populate the tree view with found sessions and files"""
    # Add Natus folders (blue)
    for session_name, folder_path in sorted(self.natus_folders.items()):
        edf_count = len([f for f in os.listdir(folder_path) 
                        if f.lower().endswith('.edf')])
        self.tree.insert('', 'end', values=(session_name, edf_count, folder_path),
                        tags=('natus',))
    
    # Add root directory EDF files (white/default)
    for filename, file_path in sorted(self.edf_files.items()):
        self.tree.insert('', 'end', values=(filename, 1, file_path),
                        tags=('root_edf',))
    
    # Add BIDS EDF files (green)
    for filename, file_info in sorted(self.bids_files.items()):
        session_info = file_info['session']
        file_path = file_info['path']
        self.tree.insert('', 'end', values=(session_info, 1, file_path),
                        tags=('bids',))
    
    # Add unexpected EDF files (red)
    for filename, file_path in sorted(self.unexpected_files.items()):
        self.tree.insert('', 'end', values=(filename, 1, file_path),
                        tags=('unexpected',))
    
    # Configure tags for colors
    self.tree.tag_configure('natus', background='lightblue')
    self.tree.tag_configure('root_edf', background='white')
    self.tree.tag_configure('bids', background='lightgreen')
    self.tree.tag_configure('unexpected', background='lightcoral')

def create_legend(self):
    """Create a legend frame explaining the color coding"""
    legend_frame = ttk.LabelFrame(self.main_frame, text="Legend", padding="5")
    legend_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
    
    legends = [
        ("Natus Folders", "lightblue"),
        ("Root Directory EDFs", "white"),
        ("BIDS EDFs", "lightgreen"),
        ("Unexpected EDFs", "lightcoral")
    ]
    
    for idx, (text, color) in enumerate(legends):
        label = tk.Label(legend_frame, text=text, background=color, 
...
class FolderRow:
    __slots__ = (
        "item_type", "selected", "status", "folder_name", "folder_path", "dominant_date", "dom_count",
        "dom_fraction", "total_files", "total_size", "has_eeg", "latest_ts",
        "study_name", "rec_start", "rec_end", "eegno", "machine"
    )
    
    def __init__(self, folder_name, folder_path, item_type="Natus"):
        self.item_type = item_type  # "Natus", "EDF", "BIDS", or "Unexpected"
        self.selected = False
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Natus Session Finder v3.0 (Enhanced with EDF & BIDS Support)")
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(frm, text="Append to list", variable=self.var_append).grid(row=0, column=9, padx=5)
        ...
        # Legend
        legend_label = ttk.Label(pfrm, text="Legend: ", font=("", 9, "bold"))
        legend_label.grid(row=0, column=2, sticky="e", padx=(20,5))
        natus_label = ttk.Label(pfrm, text="■ Natus (Black)", foreground="black", font=("", 9))
        natus_label.grid(row=0, column=3, sticky="w", padx=5)
        edf_label = ttk.Label(pfrm, text="■ EDF (Blue)", foreground="blue", font=("", 9))
        edf_label.grid(row=0, column=4, sticky="w", padx=5)
        ...
        default_columns = [
            "item_type", "selected", "status", "folder_name", "dominant_date", "dom_fraction",
        ...
        self.tree.tag_configure("natus", foreground="black")
        self.tree.tag_configure("edf", foreground="blue")
    ...
    def is_natus_folder(self, folder_path):
        """Check if a folder contains Natus recording files (.eeg or .ent)"""
        try:
            for item in os.listdir(folder_path):
                if item.lower().endswith(('.eeg', '.ent')):
                    return True
            return False
        except Exception:
            return False
    ...
    def _insert_row(self, r, recent_label: str):
        ...
        # Determine tag based on status and item type
        if r.status == "Missing":
            tag = "missing"
        elif r.status == "New":
            tag = "new"
        elif r.item_type == "EDF":
            tag = "edf"
        else:
            tag = "natus"
    ...
    def _scan_worker(self, root, prefix, days):
        try:
            root_path = Path(root)
            candidates = []  # List of (type, path) tuples
            ...
            with os.scandir(root_path) as it:
                for entry in it:
                    ...
                    # Check for Natus directories
                    if entry.is_dir():
                        name = entry.name
                        if prefix:
                            if name.lower().startswith(prefix.lower()):
                                candidates.append(("Natus", Path(entry.path)))
                        else:
                            candidates.append(("Natus", Path(entry.path)))
                    ...
                    # Check for EDF files
                        ...
                        name = entry.name
                        if any(name.lower().endswith(ext) for ext in ('.edf', '.edfz', '.bdf', '.bdfz')):
                            # Apply prefix filter to EDF files too if specified
                            if prefix:
                                if name.lower().startswith(prefix.lower()):
                                    candidates.append(("EDF", Path(entry.path)))
                            else:
                                candidates.append(("EDF", Path(entry.path)))
            ...
            natus_count = sum(1 for t, _ in candidates if t == "Natus")
            edf_count = sum(1 for t, _ in candidates if t == "EDF")
            self.log(f"Found {total} items: {natus_count} Natus folders, {edf_count} EDF files.")
            self._progress_reset(total=max(1,total), text="Scanning...")

            for idx, (item_type, item_path) in enumerate(sorted(candidates, key=lambda x: x[1].name), 1):
                if self._stop_event.is_set():
                    self.log("Scan cancelled during analysis.")
                    break

                r = FolderRow(item_path.name, item_path, item_type=item_type)
                
                if item_type == "Natus":
                    # Analyze Natus folder
                    stats = analyze_folder(item_path, log=self.log)
                    ...
                    # Analyze EDF file
                    try:
                        file_stats = item_path.stat()
                        r.total_size = file_stats.st_size
                        r.latest_ts = safe_latest_ts(item_path)
                        r.total_files = 1
                        r.has_eeg = True
                        r.status = "Present"
                        
                        # Extract dominant date from file timestamp initially
                        r.dominant_date = to_date_floor(r.latest_ts).isoformat()
                        r.dom_count = 1
                        r.dom_fraction = 1.0
                        
                        # Try to extract EDF metadata
                        if EDF_AVAILABLE:
                            edf_meta = edf_extract_metadata(item_path, log=self.log)
                            if edf_meta.get("RecordingStartTime"):
                                # Update dominant date from recording start if available
                                try:
                                    start_dt = datetime.strptime(edf_meta["RecordingStartTime"], "%Y-%m-%d %H:%M:%S")
                                    r.dominant_date = start_dt.date().isoformat()
                                except:
                                    pass
                        ...
                        self.log(f"[EDF scan error] {item_path.name}: {e}")
                        r.status = "Present"
                        raise(e)

                self.rows.append(r)
                self._insert_row(r, recent_label if item_type == "Natus" or (item_type == "EDF" and days) else "")

                self._progress_step(step=1, text=f"Scanning... {idx}/{total or 1}")
                self.log(f"[{idx}/{total}] {item_type}: {item_path.name} | files={r.total_files} | dom={r.dominant_date} | eeg={r.has_eeg}")
    ...
    def _meta_worker(self, rows):
        total = len(rows)
        self.log("Fetching quick metadata for selected items...")
        ...
        try:
            for i, r in enumerate(rows, 1):
                ...
                if r.item_type == "Natus":
                    # Extract metadata from Natus .eeg/.ent files
                    ...
                    meta = edf_extract_metadata(Path(r.folder_path), log=self.log)
                    r.study_name = meta.get("StudyName", "") or r.study_name
                    r.rec_start = meta.get("RecordingStartTime", "") or r.rec_start
                    r.rec_end = meta.get("RecordingEndTime", "") or r.rec_end
                    r.eegno = meta.get("EegNo", "") or meta.get("PatientCode", "") or r.eegno
                    r.machine = meta.get("Machine", "") or r.machine
                ...
                self._progress_step(step=1, text=f"Quick metadata... {i}/{total or 1}")
                self.log(f"[meta {i}/{total}] {r.item_type}: {r.folder_name}: StudyName='{r.study_name}' Start='{r.rec_start}' End='{r.rec_end}'")
