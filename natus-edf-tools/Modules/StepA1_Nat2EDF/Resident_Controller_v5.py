...
def run_conversion(input_folder, processed_list):
    if input_folder in processed_list:
        ...
        if os.path.exists(source_path):
            try:
                ...
                print(f"Error moving already-processed folder: {e}")
        return
    ...
    # Check if EDF file was created (by pattern, not hardcoded name)
    edf_created = any(fname.lower().endswith('.edf') for fname in os.listdir(DEST_FOLDER))
    if edf_created:
        end_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # Move the entire source folder to archive
        archive_path = os.path.join(ARCHIVE_FOLDER, input_folder)
        try:
            shutil.move(source_path, archive_path)
        except Exception as e:
            print(f"Error archiving folder {input_folder}: {e}")
            return

        # Create log inside archive folder
        ...
        print(f"No EDF file found in output folder for {input_folder}")
