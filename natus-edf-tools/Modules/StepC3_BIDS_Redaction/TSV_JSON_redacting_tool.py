...
def prompt_user_for_replacement(line, name, file):
    ...
    # Check if name appears in the line without strict word boundary checking
    if name.lower() not in tmp_line.lower():
        return False
...
def get_compiled_pattern(name):
    ...
    if len(name_parts) == 2:
        ...
        # Allow for more flexible matching including after "Montage:"
        pattern_str = (
            rf"(?:(?<=^)|(?<=[\s:]))"  # Start with start/space/colon
            rf"{re.escape(first)}[\s,_.\|;\-]*{re.escape(last)}(?=$|[\s\.,;|\-])"  # Include - in end boundary
        )
    else:
        pattern_str = (
            rf"(?:(?<=^)|(?<=[\s:]))"
            rf"{re.escape(name)}(?=$|[\s\.,;|\-])"  # Include - in end boundary
        )
