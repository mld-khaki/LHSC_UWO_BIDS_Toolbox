...
# Cache of "accept all" decisions for the exact matched text (case-sensitive result decided per token)
...
def apply_name_replacements(text: str, raw_name: str, file: str) -> (str, bool):
    """
    Apply replacements for `raw_name` inside `text`, prompting the user once per
    exact matched token and allowing an 'accept all' option for future identical tokens.
    ...
    def repl(m: re.Match) -> str:
        ...
        # Show context (ignore-list scrubbed) + exact token we intend to replace
        tmp_line = _strip_ignored(text)
        print("=" * 80)
        print(f"\nFound match upd: {tmp_line.strip()}, in file = <{file}>")
        print(f"\nFound match: {text.strip()}")
        resp = input(
            f"Replace exact token '{tok}' (from '{raw_name}') with '.X.'? "
            f"(y or enter = yes / n = no / a = accept all for this exact token): "
        ).strip().lower()

        if resp in ("", "y"):
            return ".X." if tok and tok[0].isupper() else ".x."
        if resp == "a":
            rep = ".X." if tok and tok[0].isupper() else ".x."
            APPROVED_ACCEPT_ALL[key] = rep
            return rep

        # 'n' or anything else: keep original token
        return tok
        ...
        for row in reader:
            ...
            for cell in row:
                ...
                for raw_name in matches:
                    cell, did = apply_name_replacements(cell, raw_name, file_path)
                    if did:
                        changed = True
        ...
        if isinstance(obj, dict):
            ...
            for raw_name in matches:
                if prompt_user_for_replacement(s, raw_name, file_path):
                    s, did = apply_name_replacements(s, raw_name, file_path)
