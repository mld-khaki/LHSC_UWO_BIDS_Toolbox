...
def build_automaton(names):
    """Build an Aho–Corasick automaton ensuring longer names are matched first."""
    A = ahocorasick.Automaton()
    
    # Sort names by length in descending order to ensure longer names are inserted first
...
def find_matches(text, automaton):
    ...
    # Sort matches so that longest match comes first
...
def process_tsv(file_path, args, automaton):
    ...
    with open(file_path, "r", encoding="utf-8") as infile, open(temp_file_path, "w", encoding="utf-8", newline='') as outfile:
        ...
        for row in reader:
            ...
            for cell in row:
                original_cell = cell
                matches = sorted(find_matches(cell, automaton), key=len, reverse=True)  # Sort matches by length, longest first
                for name in matches:
...
def process_json(file_path, args, automaton):
    ...
    def redact(obj):
        ...
        if isinstance(obj, dict):
            ...
            original_obj = obj
            matches = find_matches(obj, automaton)
            for name in matches:
    ...
    for root, _, files in os.walk(args.input_folder):
        for file in files:
            ...
            if ext in file_extensions:
                ...
                changed = file_extensions[ext](file_path, args, automaton)
                if changed:
                    total_changed += 1
